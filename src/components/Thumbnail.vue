<template>
  <div
    class="tw-w-full md:tw-w-fit thumbnail-container tw-flex tw-flex-wrap tw-cursor-pointer">
    <div
      class="tw-w-full md:tw-w-fit tw-bg-white tw-shadow-md tw-relative tw-overflow-hidden hover:tw-shadow-sm tw-transition-shadow tw-transition-border"
      :class="isEditing ? ['shadow-sm'] : [dragging]">
      <div
        class="thumbnail-cover | tw-absolute tw-top-0 tw-left-0 tw-flex tw-items-center tw-z-10 tw-pointer-events-none">
        <button
          v-if="editMode"
      ...
      <div
        class="thumbnail | tw-relative"
        @click="editing(groupIndex, index, true)">
        <div
          class="dsm tw-p-1 tw-w-fit tw-absolute tw-left-1/2 tw-top-1/2 tw--translate-x-1/2 tw--translate-y-1/2">
          <div
            ref="itemElement"
      ...
      <div
        v-if="item.name || item.description"
        ...
        <h4
          class="tw-m-0 tw-text-sm tw-whitespace-nowrap tw-overflow-hidden tw-overflow-ellipsis">
          {{ item.name }}
        </h4>
        <p
          class="tw-m-0 tw-font-light tw-text-sm tw-whitespace-nowrap tw-overflow-hidden tw-overflow-ellipsis">
          {{ item.description }}
        </p>
      </div>
    </div>
...
const editing = (groupIndex: number, index: number, value: boolean) =>
  emit('editing', groupIndex, index, value)

const deleteElement = (groupIndex: number, index: number) =>
  emit('delete', groupIndex, index)

const copyElement = (groupIndex: number, index: number) =>
